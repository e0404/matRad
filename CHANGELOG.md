...
- Neutron dose calculation for fast neutron therapy and BNCT: radiation mode `neutrons` with the machine files `neutrons_Generic` (MEDAPP/FRM II kernels and spectra), `neutrons_FNT`, `neutrons_BNCT` and `neutrons_thermalFRM`; the SVD pencil beam engine `matRad_NeutronPencilBeamSVDEngine` (`pln.propDoseCalc.engine = 'SVDPB'`) with a two-term depth dose parameterization and HU-binned KERMA corrections, and the Monte Carlo engine `matRad_NeutronMCNPEngine` (`'MCNP'`) that writes MCNP run files (`externalCalculation = 'write'`), runs a local `mcnp6` (`'off'`) or reads results from a folder. Neutrons use the photon stf generators and `constRBE`. See `examples/matRad_example23_neutrons.m`
...
- `matRad_LinearScalingModel.getRBEminMax`: the LET threshold mask was written as a chained `a < b < c` comparison, which MATLAB evaluates as `(a < b) < c` and is therefore always true, so the interior branch silently covered every voxel; out-of-range LET values were then extrapolated with the raw (unclamped) LET instead of the threshold LET; and the final normalization divided by `vAlphaX` indexed with that always-true mask instead of the full vector, leaving clamped voxels unnormalized. Added a regression test (`test_getRBEminMax_LinearScalingModel`)
