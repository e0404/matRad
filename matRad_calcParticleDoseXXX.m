...
for i = 1:dij.numOfBeams % loop over all beams
    ...
    for j = 1:stf(i).numOfRays % loop over all rays
        ...
        if ~isempty(stf(i).ray(j).energy)
            ...
            E = (R/alpha).^(1/p);
            sigma0 = 3;
            sigmaFineSampling = .8*sigma0;
            deltaFineSampling = .7*sigma0;
            wix = -7:7;
            offset = 0;
            nFineSamp = 19;
            myGaussian1D = @(x,mu,sigma) 1/sqrt(2*pi)/sigma * exp(-(x-mu).^2 /2/sigma^2);
            tempWeightsX  = deltaFineSampling * myGaussian1D(wix*deltaFineSampling,offset,sqrt(sigma0^2-sigmaFineSampling^2));
            weightsX(1) = tempWeightsX(8);
            weightsX(2:7) = tempWeightsX(9);
            weightsX(8:19) = tempWeightsX(10);
            ...
            for k = 1:stf(i).numOfBixelsPerRay(j) % loop over all bixels per ray
                ...
                for n=1:nFineSamp
                    ...
                    elseif n<=7
                        ...
                        targetP = stf(i).ray(j).targetPoint_bev +...
                            [4*radius*cos(ang1(n-1)),0,4*radius*sin(ang1(n-1))];
                    end
                    ...
                    if n>1
                        tempBixelDose = weightsX(n).^2.*matRad_calcParticleDoseBixel(...
                            radDepths(currIx), ...
                        ...
                        bixelDose(idxsIntoTempB) = bixelDose(idxsIntoTempB) + tempBixelDose(idxsIntoV);
                        ...
                        bixelDose = weightsX(n).^2.*matRad_calcParticleDoseBixel(...
                            radDepths(currIx), ...
                            radialDist_sq(currIx), ...
                            stf(i).ray(j).SSD, ...
                            stf(i).ray(j).focusIx(k), ...
