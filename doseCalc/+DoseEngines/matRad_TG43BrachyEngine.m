classdef matRad_TG43BrachyEngine < DoseEngines.matRad_DoseEngineBase
    ...
    methods (Access = protected)
        ...
        function dij = calcDose(this,ct,cst,stf)
            ...
            dij = this.initDoseCalc(ct, cst, stf);
                   ...
                   if strcmp(this.TG43approximation,'2D')
                       ...
                       tmpTimer = tic;
                       [ThetaMatrix,~] = matRad_getThetaMatrix(stf.templateNormal,calcDistanceMatrix);
                       matRad_cfg.dispInfo('done in %f s!\n',toc(tmpTimer));
                   end
                   ...
                   switch this.TG43approximation
                       case '1D'        
                           DoseRate(~Ignore) = ...
                           matRad_getDoseRate1D_poly(this.machine,calcDistanceMatrix.dist);
                       case '2D'
                           DoseRate(~Ignore) = ...
                           matRad_getDoseRate2D_poly(this.machine,calcDistanceMatrix.dist,ThetaMatrix);
                       otherwise
        ...
        function DoseRate = getDoseRate1D_poly(~,r_mm)
            ...
            gL = radialDoseFunction(r,gLTab);
            GL = geometryFunction(r,theta0,L);
            GL0 = geometryFunction(r0,theta0,L);
            PhiAn = anisotropyFactor1D(r,PhiAnTab, L);

            DoseRate = Sk * lambda * GL./GL0 .* gL .* PhiAn;

        end

        function DoseRate = getDoseRate2D_poly(~,r_mm,theta)
            matRad_cfg = MatRad_Config.instance();
        ...
        function templateRoot = getTemplateRoot(ct,cst)
            V = [];
