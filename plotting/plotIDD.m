...
if isstr(varargin{end}) && (strcmp(varargin{end},'RBE') || strcmp(varargin{end},'RBExD'))
    for i = 1:numel(varargin)-1
        plot(matRad_calcIDD(varargin{i}.RBExD),'LineWidth',1.5, 'DisplayName', inputname(i))
    end
    ylabel('RBExD')
else
    for i = 1:numel(varargin)
        plot(matRad_calcIDD(varargin{i}.physicalDose),'LineWidth',1.5, 'DisplayName', inputname(i))
    end
    ylabel('physicalDose')
end
xlabel('depth [voxel]')
