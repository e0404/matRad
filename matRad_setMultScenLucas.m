function multScen = matRad_setMultScenLucas(uIn)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
switch uIn.shiftCombType
    ...
    case 'direct'
        ...
        if isequal(numIso(1), numIso(2), numIso(3))
            ...
            warning('Numnber of isoShifts in every direction has to be equal in order to perform direct combination. Performing individually instead.');
...
switch uIn.rangeGenType
    ...
    otherwise
        error('Not a valid type of generating data.');
end
...
if ~isequal(numOfRelRangeShift, numOfAbsRangeShift)
    error('Number of relative and absolute range shifts must not differ.');
else
...
% do not use first scenario (scenMask(1,1,1,...) resp. linearmask(1,:)) for statistics,
% since it is included every time as the nominal scenario. this is due to
% construction with linspace.
scenProb(1) = NaN;
% call calcScenProb.
scenProb = scenProb / sum(scenProb(2:end));
...
multScen.totalNumOfScen = totalNumScen;
