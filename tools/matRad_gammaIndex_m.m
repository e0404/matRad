function [gammaCube,myColormap,gammaPassRate] = matRad_gammaIndex(cube1,cube2,resolution,criteria,slice,method,interpoints,localglobal) %additional mode(standard, interpolation...)    local and global gamma
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%   method:         paramenter to improve gamma index calculation
%                   + 'standard': default
%                   + 'linear'
%                   + 'cubic'
%                   + 'nearest'
%                   + 'spline'
%   interpoints:    number of points interpolation (see interp3
%                    documentation for more info. must be a integer
%   localglobal:    parameter that choose the kind of gamma want 
%                       to evaluate
%                   + 'global': default
%                   + 'local': 
%
...
if exist('method','var')
    if ~strcmp(method,'standard')
        ...
        cubex2 = interp3(cubex2,n,method);
    end
end
...
if flag == 1
    gammaCubeSqx = gammaCubeSq(1:2^n:end,1:2^n:end,1:2^n:end);
else
    gammaCubeSqx = gammaCubeSq;
end

gammaCubeSq = zeros(size(cube1));
gammaCubeSq(cut1,cut2,cut3) = gammaCubeSqx((1+searchX):(end-searchX), ...
            (1+searchY):(end-searchY), ...
            (1+searchZ):(end-searchZ));

gammaCube = sqrt(abs(gammaCubeSq));
...
if nargin > 4   
        ...
        if exist('method','var')
            title([num2str(gammaPassRate,5) '% of points > ' num2str(relDoseThreshold) '% passed (' num2str(relDoseThreshold) '% / ' num2str(dist2AgreeMm) 'mm)  method: ' method]);
        else
