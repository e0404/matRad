...
switch env
     ...
     case 'OCTAVE'
         fprintf(['matRad GUI not fully supported for ' env ' ' versionString ' \n']);
...
try, javax.swing.UIManager.setLookAndFeel(lf);, catch, fprintf('javax not supported\n'), end
...
% enable try, opengl software, catch, fprintf('opengl not supported\n'), end rendering to visualize linewidths properly
if ispc
  try, opengl software, catch, fprintf('opengl not supported\n'), end
elseif ismac
...
[im, ~, alpha] = imread('gui/matrad_logo.png');
f = image(im);
axis equal off
set(f, 'AlphaData', alpha);
% show dkfz logo
axes(handles.axesDKFZ)
[im, ~, alpha] = imread('gui/DKFZ_Logo.png');
f = image(im);
axis equal off;
set(f, 'AlphaData', alpha);
...
try
   ...
   handles = showError(handles,['GUI OpeningFunc: Could not load ct and cst file: ' MEexception.message]);
end
...
%Scrollbar
cstVertTableScroll = findobj(get(cstPanel,'Children'),'Style','slider');
...
if ~gui_Create
    ...
    if gui_Exported
        ...
        try, movegui(gui_hFigure,'onscreen');, catch, fprintf('movegui not supported\n'), end
    else
