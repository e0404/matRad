...
dirOut = [pwd '\standalone\build\']; % specify the output folder
...
mcc('-C','-o','matRad','-W','WinMain:matRad','-T','link:exe','-d',dirOut,...      
    ...
    '-a',[dirMatRad '\dicomImport\DKFZ_Logo.png'],'-a',[dirMatRad '\dicomImport\matrad_logo.png'],...
    '-a',[dirMatRad '\dicomImport\hlutDefault.mat'])


% note that you have to provide the generic machine data (or any other machine data)
% in the same directory for the application to run
copyfile('*_Generic.mat', dirOut)
