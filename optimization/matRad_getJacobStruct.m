...
for i = 1:size(cst,1)    
    for i = 1:numel(cst{i,6})
        if isequal(cst{i,6}(i).type, 'max mean dose constraint') || ...
           isequal(cst{i,6}(i).type, 'min mean dose constraint') || ...
           isequal(cst{i,6}(i).type, 'min max mean dose constraint')

                % -> no other max, min or min max mean dose constraint
                for j = i+1:numel(cst{i,6})
        ...
        elseif isequal(cst{i,6}(i).type, 'max EUD constraint') || ...
               isequal(cst{i,6}(i).type, 'min EUD constraint') || ...
               isequal(cst{i,6}(i).type, 'min max EUD constraint')

                % -> no other max, min or min max mean dose constraint
                for j = i+1:numel(cst{i,6})
                    if isequal(cst{i,6}(j).type, 'max EUD constraint') || ...
        ...
        elseif isequal(cst{i,6}(i).type, 'exact DVH constraint') ||...
               isequal(cst{i,6}(i).type, 'max DVH constraint') ||...
               isequal(cst{i,6}(i).type, 'min DVH constraint')

            % -> no other DVH constraint
            for j = i+1:numel(cst{i,6})
                if (isequal(cst{i,6}(j).type, 'exact DVH constraint') && isequal(cst{i,6}(i).dose,cst{i,6}(j).dose)) || ...
                   (isequal(cst{i,6}(j).type, 'exact DVH constraint') && isequal(cst{i,6}(i).volume,cst{i,6}(j).volume)) || ... 
                   (isequal(cst{i,6}(j).type, 'max DVH constraint')   && isequal(cst{i,6}(i).dose,cst{i,6}(j).dose)) || ...
                   (isequal(cst{i,6}(j).type, 'max DVH constraint')   && isequal(cst{i,6}(i).volume,cst{i,6}(j).volume)) || ... 
                   (isequal(cst{i,6}(j).type, 'min DVH constraint')   && isequal(cst{i,6}(i).dose,cst{i,6}(j).dose)) || ...
                   (isequal(cst{i,6}(j).type, 'min DVH constraint')   && isequal(cst{i,6}(i).volume,cst{i,6}(j).volume))
            ...
            if ~isempty(strfind(cst{i,6}(j).type,'constraint'))
                ...
                if strcmp(cst{i,6}(j).robustness,'none')
                    ...
                    if isequal(cst{i,6}(j).type, 'max dose constraint') || ...
                       ...
                       jacobStruct = [jacobStruct; spones(mean(dij.physicalDose{i}(cst{i,4},:)))];
