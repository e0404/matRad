...
if ~isfield(apertureInfo.beam(1),'optimizeBeam')
    ...
    j_lfspd_t = -reshape([repelem(abs(diff(reshape(leftLeafPos,apertureInfo.beam(1).numOfActiveLeafPairs,apertureInfo.totalNumOfShapes),1,2)),1,2).*repmat(timeFac',19,1) ...
        repelem(abs(diff(reshape(rightLeafPos,apertureInfo.beam(1).numOfActiveLeafPairs,apertureInfo.totalNumOfShapes),1,2)),1,2).*repmat(timeFac',19,1)]./ ...
