function hessian = matRad_hessianFuncWrapper_old(w,sigma,lambda,dij,cst,options)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        for j = 1:numel(cst{i,6})
            ...
            if isempty(strfind(cst{i,6}(j).type,'constraint'))
                ...
                if strcmp(cst{i,6}(j).robustness,'none')
                    ...
                    d_i = d{1}(cst{i,4}{1});              
                    objectiveHessian = objectiveHessian + matRad_hessianFunc_old(dij,d_i,cst{i,6}(j),cst{i,4}{1},d_ref);                    
                else
                ...
                if strcmp(cst{i,6}(j).robustness,'none')
                    ...
                    if isequal(cst{i,6}(j).type, 'max dose constraint (exact)') || isequal(cst{i,6}(j).type, 'min dose constraint (exact)')
                        ...
                        constraintHessian = constraintHessian + lambda(constraintCounter) * matRad_hessianFunc_old(dij,d_i,cst{i,6}(j),cst{i,4}{1},d_ref);
                    end
