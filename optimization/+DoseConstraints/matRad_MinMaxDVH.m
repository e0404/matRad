 classdef matRad_MinMaxDVH < DoseConstraints.matRad_DoseConstraint
    ...
    methods
        function constr = matRad_MinMaxDVH(dRef,vMin,vMax)
            ...
            if ~initFromStruct                
                if nargin == 3 && isscalar(vMax)
                    constr.parameters{3} = vMax;
                end
                
                if nargin >= 1 && isscalar(dRef)
                    constr.parameters{1} = dRef;
                end
                
                if nargin >= 2 && isscalar(vMin)
                    constr.parameters{2} = vMin;
                end
            end
        end
        ...
        function cu = upperBounds(constr,n)
            cu = constr.parameters{3} / 100;
        end
        function cl = lowerBounds(constr,n)
            cl = constr.parameters{2} / 100;
        end
        ...
        function cDose = computeDoseConstraintFunction(constr,dose)
            ...
            cDose = sum(dose >= constr.parameters{1})/numel(dose);
        ...
        function cDoseJacob  = computeDoseConstraintJacobian(constr,dose)
            ...
            % calculate scaling
            NoVoxels     = max(constr.voxelScalingRatio*numel(dose),10);
            absDiffsort  = sort(abs(constr.parameters{1} - dose_sort));                       
            
            deltaDoseMax = absDiffsort(min(ceil(NoVoxels/2),numel(dose)));
            
            % calclulate DVHC scaling
            DVHCScaling = min((log(1/constr.referenceScalingVal-1))/(2*deltaDoseMax),250);
            
            d_diff = dose - constr.parameters{1};
