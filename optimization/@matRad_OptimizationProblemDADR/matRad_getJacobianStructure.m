function jacobStruct = matRad_getJacobianStructure(optiProb,~,dij,cst)	
...
jacobStruct = sparse([]);
jacobStructDose = sparse([]);
jacobStructDoseRate = sparse([]);
        ...
        for j = 1:numel(cst{i,6})	
            ...
            % only perform computations for constraints	
              if isa(obj,'DoseConstraints.matRad_DoseConstraint')
                ...
                helper = zeros([nRows size(dij.physicalDose{1},2)]);

                jacobStructDose = [jacobStructDose; [repmat(spones(mean(dij.physicalDose{1}(cst{i,4}{1},:))),nRows,1) helper]];
                
              elseif isa(obj,'DADRConstraints.matRad_DADRConstraint')
                  % get the jacobian structure depending on dose-rate	
                jacobDoseStruct = obj.getDoseConstraintJacobianStructure(numel(cst{i,4}{1}));	
                nRows = size(jacobDoseStruct,2);	
%                 helper = zeros([nRows size(dij.physicalDose{1},2)]);
                helper = repmat(spones(mean(dij.physicalDose{1}(cst{i,4}{1},:))),nRows,1);
                jacobStructDoseRate = [jacobStructDoseRate; [helper helper]];
...
jacobStruct = [jacobStructDose; jacobStructDoseRate];
