    ...
    if updatedInfo.beam(i).numOfShapes ~= 0 %numOfShapes is 0 for interpolated beams!
        ...
        for j = 1:updatedInfo.beam(i).numOfShapes
            ...
            xPosIndRightLeaf(rightLeafPos == apertureInfo.beam(i).lim_r) = ...
                ...
                leftLeafPosI = min([updatedInfo.beam(i).shape(j).leftLeafPos_I,updatedInfo.beam(i).shape(j).leftLeafPos_F],[],2);
                leftLeafPosF = max([updatedInfo.beam(i).shape(j).leftLeafPos_I,updatedInfo.beam(i).shape(j).leftLeafPos_F],[],2);
                rightLeafPosI = min([updatedInfo.beam(i).shape(j).rightLeafPos_I,updatedInfo.beam(i).shape(j).rightLeafPos_F],[],2);
                ...
                xPosIndLeftLeafI  = round((leftLeafPosI - apertureInfo.beam(i).posOfCornerBixel(1))/apertureInfo.bixelWidth + 1);
                xPosIndLeftLeafF  = round((leftLeafPosF - apertureInfo.beam(i).posOfCornerBixel(1))/apertureInfo.bixelWidth + 1);
                xPosIndRightLeafI = round((rightLeafPosI - apertureInfo.beam(i).posOfCornerBixel(1))/apertureInfo.bixelWidth + 1);
                xPosIndRightLeafF = round((rightLeafPosF - apertureInfo.beam(i).posOfCornerBixel(1))/apertureInfo.bixelWidth + 1);
                ...
                xPosLinearIndLeftLeafI = sub2ind([n size(edges_l)],(1:n)',xPosIndLeftLeafI);
                xPosLinearIndLeftLeafF = sub2ind([n size(edges_l)],(1:n)',xPosIndLeftLeafF);
                xPosLinearIndRightLeafI = sub2ind([n size(edges_l)],(1:n)',xPosIndRightLeafI);
                xPosLinearIndRightLeafF = sub2ind([n size(edges_l)],(1:n)',xPosIndRightLeafF);
                ...
                %initial computation
                uncoveredByLeftLeaf = bsxfun(@minus,(edges_l+edges_r)/2,leftLeafPosI)./repmat(leftLeafPosF-leftLeafPosI,1,size(edges_l,2));
                %correct for overshoot in initial and final leaf positions
                uncoveredByLeftLeaf(xPosLinearIndLeftLeafI) = uncoveredByLeftLeaf(xPosLinearIndLeftLeafI)+(leftLeafPosI-edges_l(xPosIndLeftLeafI)').^2./((leftLeafPosF-leftLeafPosI).*(edges_r(xPosIndLeftLeafI)'-edges_l(xPosIndLeftLeafI)').*2);
                uncoveredByLeftLeaf(xPosLinearIndLeftLeafF) = uncoveredByLeftLeaf(xPosLinearIndLeftLeafF)-(edges_r(xPosIndLeftLeafF)'-leftLeafPosF).^2./((leftLeafPosF-leftLeafPosI).*(edges_r(xPosIndLeftLeafF)'-edges_l(xPosIndLeftLeafF)').*2);
                %round <0 to 0, >1 to 1
                uncoveredByLeftLeaf = (uncoveredByLeftLeaf+abs(uncoveredByLeftLeaf))/2;
                uncoveredByLeftLeaf = (uncoveredByLeftLeaf+1-abs(uncoveredByLeftLeaf-1))/2;
                
                %calculate fraction of fluence covered by right leaf
                %initial computation
                coveredByRightLeaf = bsxfun(@minus,(edges_l+edges_r)/2,rightLeafPosI)./repmat(rightLeafPosF-rightLeafPosI,1,size(edges_l,2));
                %correct for overshoot in initial and final leaf positions
                coveredByRightLeaf(xPosLinearIndRightLeafI) = coveredByRightLeaf(xPosLinearIndRightLeafI)+(rightLeafPosI-edges_l(xPosIndRightLeafI)').^2./((rightLeafPosF-rightLeafPosI).*(edges_r(xPosIndRightLeafI)'-edges_l(xPosIndRightLeafI)').*2);
                coveredByRightLeaf(xPosLinearIndRightLeafF) = coveredByRightLeaf(xPosLinearIndRightLeafF)-(edges_r(xPosIndRightLeafF)'-rightLeafPosF).^2./((rightLeafPosF-rightLeafPosI).*(edges_r(xPosIndRightLeafF)'-edges_l(xPosIndRightLeafF)').*2);
                %round <0 to 0, >1 to 1
                coveredByRightLeaf = (coveredByRightLeaf+abs(coveredByRightLeaf))/2;
                coveredByRightLeaf = (coveredByRightLeaf+1-abs(coveredByRightLeaf-1))/2;
                ...
                tempMap = round2(tempMap,10);
                tempMap(isnan(tempMap)) = 0;
                
                tempMap_NEW_I = uncoveredByLeftLeaf_I-coveredByRightLeaf_I;
                tempMap_NEW_F = uncoveredByLeftLeaf_F-coveredByRightLeaf_F;
                tempMap_NEW_I = round2(tempMap_NEW_I,10);
                tempMap_NEW_F = round2(tempMap_NEW_F,10);
                tempMap_NEW_I(isnan(tempMap_NEW_I)) = 0;
                tempMap_NEW_F(isnan(tempMap_NEW_F)) = 0;
                ...
                tempMap_NEW = weight_I.*tempMap_NEW_I+weight_F.*tempMap_NEW_F;
