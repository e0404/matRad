...
for i = 1:numel(updatedInfo.beam)
    ...
    if updatedInfo.beam(i).numOfShapes ~= 0 %numOfShapes is 0 for interpolated beams!
        ...
        if ~exist('leftLeafPoss','var') && isfield(apertureInfo,'gantryRotCst')
            ...
            %FIX THIS
            borderGantryAngles = nan(1,numel(updatedInfo.beam));
            ...
            for k = 1:numel(updatedInfo.beam)
                ...
                if k == 1
                    ...
                    borderGantryAngles(m) = updatedInfo.beam(k).lastBorderAngle;
                    if updatedInfo.beam(k).lastBorderAngle_border == 1
                        %This means that the current arc sector is moving
                        %in the normal direction (L-R).
                        borderLeftLeafPoss(:,m) = updatedInfo.beam(k).lim_l;
                    elseif updatedInfo.beam(k).lastBorderAngle_border == -1
                        %This means that the current arc sector is moving
                ...
                elseif k == numel(updatedInfo.beam)
                    %This gives starting angle of the current sector.
                    borderGantryAngles(m) = updatedInfo.beam(k).nextBorderAngle;
                    if updatedInfo.beam(k).nextBorderAngle_border == 1
                        %This means that the current arc sector is moving
                        %in the normal direction (L-R).
                        borderLeftLeafPoss(:,m) = updatedInfo.beam(k).lim_l;
                    elseif updatedInfo.beam(k).nextBorderAngle_border == -1
                        %This means that the current arc sector is moving
                        %in the reverse direction (R-L).
                        borderLeftLeafPoss(:,m) = updatedInfo.beam(k).lim_r;
                    end
                ...
                elseif ~isempty(updatedInfo.beam(k).border)
                    %This gives starting angle of the current sector.
                    borderGantryAngles(m) = updatedInfo.beam(k).gantryAngle;
                    if updatedInfo.beam(k).border == 1
                        %This means that the current arc sector is moving
                        %in the normal direction (L-R).
                        borderLeftLeafPoss(:,m) = updatedInfo.beam(k).lim_l;
                    elseif updatedInfo.beam(k).border == -1
                        %This means that the current arc sector is moving
                        %in the reverse direction (R-L).
                        borderLeftLeafPoss(:,m) = updatedInfo.beam(k).lim_r;
                    end
            ...
            borderGantryAngles(isnan(borderGantryAngles)) = [];
            ...
            for row = 1:dimZ
                ...
                elseif ~isempty(touchingInd)
                    ...
                    gantryAnglesAug = [optGantryAngles,borderGantryAngles];
                    leftLeafPossAug = [reshape(mean([leftLeafPoss(:) rightLeafPoss(:)],2),size(leftLeafPoss)),borderLeftLeafPoss];
                    %leftLeafPossAug = leftLeafPossAug(:,ind);
                    notTouchingInd = setdiff(1:updatedInfo.totalNumOfShapes,touchingInd);
                    notTouchingIndAug = [notTouchingInd,(1+numel(optGantryAngles)):(numel(optGantryAngles)+numel(borderGantryAngles))];
                    %notTouchingIndAug = notTouchingIndAug(ind);
                    ...
                    %HAVE TO UPDATE ACTUAL OPTIMIZED BEAMS LEAF POSITIONS
                    %TOO!
                end
            end
        end
        ...
        if ~exist('leftLeafPoss','var')
            ...
            for k = 1:numel(updatedInfo.beam)
                ...
                if ~isempty(updatedInfo.beam(k).border)
                    %Only important when cleaning up instances of opposing
                    %leaves touching.
                    %This gives starting angle of the current sector.
                    borderGantryAngles(m) = updatedInfo.beam(k).gantryAngle;
            ...
            for row = 1:dimZ
                ...
                elseif ~isempty(touchingInd)
                    ...
                    %THERE IS AN ISSUE HERE WITH THE FIRST ANGLE BEING A
                    %BORDER AND AN OPT!
                    leftLeafPoss(row,touchingInd) = interp1(gantryAnglesAug(notTouchingIndAug),leftLeafPossAug(row,notTouchingIndAug),optGantryAngles(touchingInd));
                    rightLeafPoss(row,touchingInd) = leftLeafPoss(row,touchingInd);
