...
if ~isempty(optiProb.dirtyDoseBP)
    ...
    d = optiProb.dirtyDoseBP.GetResult();

    % also get probabilistic quantities (nearly no overhead if empty)
    [dExp,dOmega] = optiProb.dirtyDoseBP.GetResultProb();

    % get the used scenarios
    useScen  = optiProb.dirtyDoseBP.scenarios;
    scenProb = optiProb.dirtyDoseBP.scenarioProb;
    useNominalCtScen = optiProb.dirtyDoseBP.nominalCtScenarios;
...
for  i = 1:size(cst,1)
    ...
    if ~isempty(cst{i,4}{1}) && ( isequal(cst{i,3},'OAR') || isequal(cst{i,3},'TARGET') )
        ...
        for j = 1:numel(cst{i,6})
         ...
         elseif ~isempty(optiProb.dirtyDoseBP) && isa(objective,'DirtyDoseObjectives.matRad_DirtyDoseObjective')
              % only perform gradient computations for objectives
                ...
                switch robustness
                    ...
                    case 'COWC' % composite worst case consideres ovarall the worst objective function value
                        ...
                        for s = 1:numel(useScen)
                            ...
                            f_COWC(ixScen) = f_COWC(ixScen) + objective.penalty*objective.computeDirtsDoseObjectiveFunction(d_i);
