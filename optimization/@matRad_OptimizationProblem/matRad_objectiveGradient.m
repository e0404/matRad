    ...
    if ~isempty(cst{i,4}{1}) && ( isequal(cst{i,3},'OAR') || isequal(cst{i,3},'TARGET') )
        ...
        for j = 1:numel(cst{i,6})
            ...
            if isa(objective,'XBDDADRObjectives.matRad_XBDDADRObjective')
                ...
                %add to XBDDADR gradient
                XBDDADRGradient{1}(cst{i,4}{1}) = XBDDADRGradient{1}(cst{i,4}{1}) + objective.computeXBDDADRObjectiveGradient(XBDDADR_i);
            ...
            if isa(objective,'XBDLETObjectives.matRad_XBDLETObjective')
                ...
                %add to XBD(LET) gradient
                XBDLETGradient{1}(cst{i,4}{1}) = XBDLETGradient{1}(cst{i,4}{1}) + objective.computeXBDLETObjectiveGradient(XBDLET_i);
...
if ~isempty(optiProb.BP_XBDDADR)
    optiProb.BP_XBDDADR = optiProb.BP_XBDDADR.computeGradient(dij,XBDDADRGradient,w);
    g_XBDDADR =  optiProb.BP_XBDDADR.GetGradient();

    weightGradient = weightGradient + g_XBDDADR{1};
end

if ~isempty(optiProb.BP_XBDLET)
    optiProb.BP_XBDLET = optiProb.BP_XBDLET.computeGradient(dij,XBDLETGradient,w);
    g_XBDLET =  optiProb.BP_XBDLET.GetGradient();

    weightGradient = weightGradient + g_XBDLET{1};
end
