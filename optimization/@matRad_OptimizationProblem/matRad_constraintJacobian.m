    ...
    if ~isempty(cst{i,4}{1}) && ( isequal(cst{i,3},'OAR') || isequal(cst{i,3},'TARGET') )
        ...
        for j = 1:numel(cst{i,6})
            ...
            if isa(obj,'DADRConstraints.matRad_DADRConstraint')
                ...
                jacobSub = obj.computeDADRConstraintJacobian(DADR_i);
...
if isa(optiProb.BP,'matRad_DoseProjection')
    ...
    if ~isempty(DoseProjection{1})
        jacob = [jacob; DoseProjection{1}' * dij.physicalDose{1}];
    end

elseif isa(optiProb.BP,'matRad_ConstantRBEProjection')

    if ~isempty(DoseProjection{1})
        jacob = [jacob; DoseProjection{1}' * dij.RBE * dij.physicalDose{1}];
    end
...
elseif isa(optiProb.BP,'matRad_EffectProjection')
    ...
    if ~isempty(mSqrtBetaDoseProjection{1}) && ~isempty(mAlphaDoseProjection{1})
        ...
        jacob = [jacob; mAlphaDoseProjection{1}' * dij.mAlphaDose{1} +...
...
if ~isempty(optiProb.BP_DADRfixed)
    ...
    ix = dose > 0;
    tmp = zeros(size(dose));
    tmp(ix) = DADRProjection{1}(ix,:) ./ dose(ix).^2;

    wJacob = (tmp.*dose)' * dij.physicalDose{scen}.^2 - (tmp.*dijSqW)' * dij.physicalDose{scen};

    jacob = [jacob; I*wJacob];
end
