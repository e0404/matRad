        ...
        for j = 1:numel(cst{i,6})
            ...
            if isa(obj,'DoseConstraints.matRad_DoseConstraint')
                ...
                %if strcmp(cst{i,6}{j}.robustness,'none')
                    ...
                    scenID2 = [scenID2;ones(numel(cst{i,4}{ctScen}),1)];
                    ...
                    for c = 1:size(jacobSub,2)
                        ...
                        if isa(optiProb.BP,'matRad_DoseProjection') && ~isempty(jacobVec) || isa(optiProb.BP,'matRad_ConstantRBEProjection')
                            ...
                            DoseProjection          = [DoseProjection,sparse(cst{i,4}{ctScen},1,jacobVec,dij.doseGrid.numOfVoxels,1)];
                            
                        elseif isequal(optiProb.BP,'matRad_VariableRBEProjection') && ~isempty(jacobVec)
                            
                            scaledEffect = (dij.gamma(cst{i,4}{ctScen}) + d_i);
                            
                            delta = jacobVec./(2*dij.bx(cst{i,4}{ctScen}).*scaledEffect);
                            
                            mAlphaDoseProjection    = [mAlphaDoseProjection,sparse(cst{i,4}{ctScen},1,delta,dij.doseGrid.numOfVoxels,1)];
                            mSqrtBetaDoseProjection = [mSqrtBetaDoseProjection,...
                                sparse(cst{i,4}{ctScen},1:numel(cst{i,4}{ctScen}),2*delta,dij.doseGrid.numOfVoxels,numel(cst{i,4}{ctScen}))];
                            voxelID                 = [voxelID ;cst{i,4}{ctScen}];
                            constraintID            = [constraintID, repmat(1 + constraintID(end),1,numel(cst{i,4}{ctScen}))];
                           
                        
                        elseif isa(optiProb.BP,'matRad_EffectProjection') && ~isempty(jacobVec)
                            
                            mAlphaDoseProjection    = [mAlphaDoseProjection,sparse(cst{i,4}{ctScen},1,jacobVec,dij.doseGrid.numOfVoxels,1)];
                            mSqrtBetaDoseProjection = [mSqrtBetaDoseProjection,...
                                sparse(cst{i,4}{ctScen},1:numel(cst{i,4}{ctScen}),2*jacobVec,dij.doseGrid.numOfVoxels,numel(cst{i,4}{ctScen}))];
                            voxelID                 = [voxelID ;cst{i,4}{ctScen}];
                            constraintID            = [constraintID, repmat(1 + constraintID(end),1,numel(cst{i,4}{ctScen}))];
