...
switch simPropMNCP.geometryOption
    case 'Lattice'
        ...
        universeMatrix = zeros(ct.cubeDim, 'uint8');    % create a 3D matrix and fill with material/universe indices
        % note: every universe corresponds to one material
        for counterMaterial = 1:size(ct.tissueBin,2)
            universeMatrix(ct.tissueBin(counterMaterial).linIndVol) = counterMaterial;  % fill
        end
        ...
        for cellCounter = 1:size(ct.tissueBin,2)
            if ~isnan(mean(ct.density{1,1}(ct.tissueBin(cellCounter).linIndVol)))
                if cellCounter==1
                    fprintf(fileID_A, blockA.universeSpec, ...
                        cellCounter, ...        % cell number
                        cellCounter, ...        % material number
                        -mean(ct.density{1,1}(ct.tissueBin(cellCounter).linIndVol)), ...    % material density
            ...
            elseif isnan(mean(ct.density{1,1}(ct.tissueBin(cellCounter).linIndVol)))
                if cellCounter==1
                    fprintf(fileID_A, blockA.universeSpec, ...
                        cellCounter, ...        % cell number
                        cellCounter, ...        % material number
        ...
        fprintf(fileID_A, blockA.latticeSpec, ...
            ...
            ct.cubeDim(1)-1, ...
            ct.cubeDim(3)-1);
        for i3 = 1:ct.cubeDim(3) % write universe index for each cell
            for i1 = 1:ct.cubeDim(1)
                for i2 = 1:ct.cubeDim(2)
        ...
        fprintf(fileID_B, blockB.universeVoxelSpec, ...
                0.25*ct.resolution.x_resized, ...
                1.75*ct.resolution.x_resized, ...
                0.25*ct.resolution.y_resized, ...
                1.75*ct.resolution.y_resized, ...
                0.25*ct.resolution.z_resized, ...
                1.75*ct.resolution.z_resized);
        
        % Write surfaces for lattice orientation: lattice cell (0,0,0)
        fprintf(fileID_B, ['505 PX ', num2str(ct.resolution.x_resized*1.5), ' $ defines x-direction\n']);
        fprintf(fileID_B, ['506 PX ', num2str(ct.resolution.x_resized/2), '\n']);
        fprintf(fileID_B, ['507 PY ', num2str(ct.resolution.y_resized*1.5), '$ defines y-direction\n']);
        fprintf(fileID_B, ['508 PY ', num2str(ct.resolution.y_resized/2), '\n']);
        fprintf(fileID_B, ['509 PZ ', num2str(ct.resolution.z_resized*1.5), '$ defines z-direction\n']);
        fprintf(fileID_B, ['510 PZ ', num2str(ct.resolution.z_resized/2), '\n']);
        ...
        fprintf(fileID_B, ['911 RPP ', ... 
            num2str(latticeMargin+ct.resolution.x_resized/2), ' ',...
            num2str(ct.cubeDim(2)*ct.resolution.x_resized+ct.resolution.x_resized/2-latticeMargin),' ',...
            num2str(latticeMargin+ct.resolution.y_resized/2), ' ',...
            num2str(ct.cubeDim(1)*ct.resolution.y_resized+ct.resolution.y_resized/2-latticeMargin), ' ',...
            num2str(latticeMargin+ct.resolution.z_resized/2), ' ',...
            num2str(ct.cubeDim(3)*ct.resolution.z_resized+ct.resolution.z_resized/2-latticeMargin),'\n']);
        ...
        fprintf(fileID_B, ['912 RPP ',num2str(-(ct.resolution.x_resized/2+additionalLayerThickness)), ' ',...
             num2str(ct.cubeDim(2)*ct.resolution.x_resized-ct.resolution.x_resized/2+additionalLayerThickness), ' ',...
             num2str(-(ct.resolution.y_resized/2+additionalLayerThickness)), ' ',...
             num2str(ct.cubeDim(1)*ct.resolution.y_resized-ct.resolution.y_resized/2+additionalLayerThickness), ' ', ...
             num2str(-(ct.resolution.z_resized/2+additionalLayerThickness)), ' ', ...
             num2str(ct.cubeDim(3)*ct.resolution.z_resized-ct.resolution.z_resized/2+additionalLayerThickness), '\n']);
