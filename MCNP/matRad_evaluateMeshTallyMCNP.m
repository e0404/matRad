function doseMatrix = matRad_evaluateMeshTallyMCNP(stf, fileName, pln, binIntervals, ct)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Evaluate files containing tally results of MCNP calculation.
%
% call
%   doseMatrix = matRad_evaluateMeshTallyMCNP(stf, fileName, tallyType, binIntervals, ctData)
%
% input
%   stf:            stearing information from MATRAD
...
%                                   'TotalDose_TMESH'
%   binIntervals:   info about HU intervals used for segmentation
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% A) FMESH Tally for KERMA
%  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% F4 tallies are used to tally flux multiplied by KERMA factors. Since
% there are specific KERMA factors for each material used in the
% Simulation, as many tallies as used materials need to be used for each
% particle species.
% Note: In MCNP all dimensions are given in cm! -> Convert to mm for use in
% MATLAB.
...
if strcmp(pln.propMCNP.tallySpecifier, 'KERMA_F4')
    ...
    % Allocate neutron KERMA matrix
    doseMatrix.neutronDose = zeros(ct.cubeDim);
    doseMatrix.neutronDose_relError = zeros(ct.cubeDim);

    
    % Read Neutron Data
    if (strcmp(stf(1).radiationMode, 'neutrons') || strcmp(stf(1).radiationMode, 'neutrons_PLUS_photons'))
        for counter=1:size(binIntervals,2)-1
            tallyLine(counter) = find(~cellfun(@isempty, strfind(textData, ['Mesh Tally Number      ', int2str(counter) ,'014'])));
            while ~strcmp(textData(tallyLine(counter)), 'Energy         X         Y         Z     Result     Rel Error')
                tallyLine(counter) = tallyLine(counter)+1;
            end
        end
        
        % Read neutron data from all tally outputs and assign KERMA to every voxel
        for counter=1:size(binIntervals,2)-1 % counter goes through materials from segmentation
            resultMCNP = zeros(prod(ct.cubeDim),5);
            dataHelper = textscan(fid_read, '%f %f %f %f %f %f %*[^\n]', 'HeaderLines', tallyLine(counter));
            resultMCNP(:,1) = dataHelper{1,2};  % Coordinates
            resultMCNP(:,2) = dataHelper{1,3};
            resultMCNP(:,3) = dataHelper{1,4};
            resultMCNP(:,4) = dataHelper{1,5};  % Value
            resultMCNP(:,5) = dataHelper{1,6};  % Rel. Error
            frewind(fid_read)
            
            resultMCNP(:,1:3) = resultMCNP(:,1:3)*10; %rescale voxel position from cm in MCNP to mm to use in MATLAB
            resultMCNP(:,4) = resultMCNP(:,4)./pln.propMCNP.normalizationFactor;    % to avoid problems with double precision, KERMA factors were multiplied by a factor, undo it here
            
            linearIndex_Tally = matRad_cart2linIndex(resultMCNP,ct); %calculate linear index of each tally voxel
            [~, linInd_in_resultMCNP, ~] = intersect(linearIndex_Tally, ct.tissueBin(counter).linIndVol, 'stable'); %find voxels assigned to material number
            doseMatrix.neutronDose(linearIndex_Tally(linInd_in_resultMCNP))=resultMCNP(linInd_in_resultMCNP,4); %fill dose matrix with colculated KERMA for considered material
            doseMatrix.neutronDose_relError(linearIndex_Tally(linInd_in_resultMCNP))=resultMCNP(linInd_in_resultMCNP,5); %fill dose matrix with relative error
        end
        
    end
    
    % Allocate photon KERMA matrix
    doseMatrix.photonDose = zeros(ct.cubeDim);
    doseMatrix.photonDose_relError = zeros(ct.cubeDim);

    
    % Read Photon Data
    if (strcmp(stf(1).radiationMode, 'photons') || strcmp(stf(1).radiationMode, 'neutrons_PLUS_photons'))
        for counter=1:size(binIntervals,2)-1
            tallyLine(counter) = find(~cellfun(@isempty, strfind(textData, ['Mesh Tally Number      ', int2str(counter) ,'024'])));
            while ~strcmp(textData(tallyLine(counter)), 'Energy         X         Y         Z     Result     Rel Error')
                tallyLine(counter) = tallyLine(counter)+1;
            end
        end
        
        % Read photon data from all tally outputs and assign KERMA to every voxel
        for counter=1:size(binIntervals,2)-1 % counter goes through materials from segmentation
            resultMCNP = zeros(prod(ct.cubeDim),5);
            dataHelper = textscan(fid_read, '%f %f %f %f %f %f %*[^\n]', 'HeaderLines', tallyLine(counter));
            resultMCNP(:,1) = dataHelper{1,2};  % Coordinates
            ...
            linearIndex_Tally = matRad_cart2linIndex(resultMCNP,ct); %calculate linear index of each tally voxel
            [~, linInd_in_resultMCNP, ~] = intersect(linearIndex_Tally, ct.tissueBin(counter).linIndVol, 'stable'); %find voxels assigned to material number
            doseMatrix.photonDose(linearIndex_Tally(linInd_in_resultMCNP))=resultMCNP(linInd_in_resultMCNP,4); %fill dose matrix with calculated KERMA for considered material
            doseMatrix.photonDose_relError(linearIndex_Tally(linInd_in_resultMCNP))=resultMCNP(linInd_in_resultMCNP,5); %fill dose matrix with calculated KERMA for considered material
        end
    end
...
%     fclose(fid_read);
    ...
    % Finish up dose matrix, neutron and photon dose are set to zero
    % since only total dose can be calculated
    doseMatrix.photonDose = zeros(ct.cubeDim);
    doseMatrix.photonDose_relError = zeros(ct.cubeDim);
    doseMatrix.neutronDose = zeros(ct.cubeDim);
    doseMatrix.neutronDose_relError = zeros(ct.cubeDim);
...
%             [doseMatrix.physicalDose_photonHeating, doseMatrix.physicalDose_photonHeating_relError] = convertDose4RBECalculation(ct, tallyData.tally1116);            
    ...
    error('Type of tally was not assigend! Donnot know how to proceed...')
end
