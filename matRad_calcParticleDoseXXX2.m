function dij = matRad_calcParticleDoseXXX2(ct,stf,pln,cst,calcDoseDirect)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for j = 1:stf(i).numOfRays % loop over all rays
        ...
        if ~isempty(stf(i).ray(j).energy)
            ...
            [finalWeight, X1, sigma_sub, radius, posx, posz, numOfSub] = ...
                matRad_calcWeights(machine.data(energyIx).initFocus.sigma(1), 2, 'circle');
            
            % run over components
            for c = 1:numOfSub
                
                [idx,idx_shift] = mR_shift(V(ix), ct.cubeDim, stf(i).sourcePoint_bev,...
                    stf(i).ray(j).targetPoint_bev, stf.isoCenter, posx(c), posz(c));
                ...
                for k = 1:stf(i).numOfBixelsPerRay(j) % loop over all bixels per ray
                    ...
                    if c>1
                        ...
                        [~,idxsIntoTempB] = intersect(superIdx,idx_shift);
                        [~,idxsIntoV] = intersect(idx_shift,superIdx);
