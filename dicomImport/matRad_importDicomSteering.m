...
% numOfBeams = length(BeamSeqNames);
% Number of Beams readed from plan
...
for i = 1:length(BeamSeqNames)
    ...
    % stf(i).SAD           = double(currBeamSeq.VirtualSourceAxisDistances(1));
    stf(i).SAD           = pln.SAD;
...
%     stf(currBeam).isoCenter     = ControlPointSeq.Item_1.IsocenterPosition.';
    ...
    StfTmp = zeros(0,4);
    for currContr = 1:numOfContrPointSeq
%         currContrSeq = getfield(ControlPointSeq,ControlPointSeqNames{currContr});
        currContrSeq = ControlPointSeq.(ControlPointSeqNames{currContr});
...
%         w = [w stf(currBeam).ray(j).weight];
    ...
    bixelWidth(1) = bixelWidth_help1(size(bixelWidth_help1,1)) - ...
        bixelWidth_help1(size(bixelWidth_help1,1)-1);
    bixelWidth(2) = bixelWidth_help2(size(bixelWidth_help2,1)) - ...
        bixelWidth_help2(size(bixelWidth_help2,1)-1);
    if bixelWidth(1) ~= bixelWidth(2)
        warning('Bixelwidth differ in the different directions');
    end
    stf(i).bixelWidth = max(bixelWidth);
    if isfield(pln,'bixelWidth') && ~isnan(pln.bixelWidth)
        if stf(i).bixelWidth ~= pln.bixelWidth
            error('multiple non-equal bixelWidths, not yet supported')
        end
    end
    pln.bixelWidth = stf(i).bixelWidth;
    % compute coordinates in lps coordinate system, i.e. rotate beam
    ...
    stf(i).timeStamp = clock;
end
pln.bixelWidth = max(stf(:).bixelWidth);
end