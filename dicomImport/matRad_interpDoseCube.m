...
% match dose cube to ct cube
% dim_ct = size(ct.cube);

% convert dosedata to 3-D cube
dose.cube = dosedata(:,:,1,1);
% number of slices
nslice = size(dosedata,4);
for i = 2:nslice
    dose.cube(:,:,i) = dosedata(:,:,1,i);
end

% not needed? convert to double
% dose.cube = im2double(dose.cube);
...
% dose.dicomInfo.Rows                    = info.Rows;
% dose.dicomInfo.Columns                 = info.Columns;

% for i = 1:size(dose.cube,3)
% imshow(dose.cube(:,:,i))
% shg
% pause(0.05)
% end
