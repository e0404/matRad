...
elseif isfield(planInfo, 'IonBeamSequence')
    ...
    errordlg('Not supported kind of DICOM file.');    
end
...
elseif (strncmpi(radiationMode,'ion',3) && radiationMass == 12 && radiationAtomicNumber == 6)
    ...
    warning('The given type of radiation may not yet supported');
end

SAD = NaN;
% load machine
% temporarily set to HIT or Generic for developing reasons
if strcmp(radiationMode,'photons')
    machineName = 'Generic';
else
    machineName = 'HIT';
end
fileName = [radiationMode '_' machineName];
try
   load(fileName);
   SAD = machine.meta.SAD;
catch
   warndlg(['Could not find the following machine file: ' fileName ]);
   machineName = 'Generic';
   fileName = [radiationMode '_' machineName];
   load(fileName);
end
