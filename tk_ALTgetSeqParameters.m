function shapeInfo = tk_ALTgetSeqParameters(seqResult,pln,stf,visBool)
    ...
    for i=1:pln.numOfBeams
        ...
        for j=1:seqResult.beam(i).numOfShapes
            ...
            if j==1
                ...
                [shapeLim_l, shapeLim_r] = matRad_getLeafPos(rayMx);
            end


            tempShape = seqResult.beam(i).shapes(:,:,j);
            [Ix_l, Ix_r, minIx, maxIx] = matRad_getLeafPos(tempShape);

            %visualize shape
            if visBool
                figure
                titleString = ['BeamNr: ' int2str(i) ' ShapeNr: ' int2str(j)];
                matRad_visSingleShape(tempShape,Ix_l,Ix_r,titleString)
