...
% how to setup a photon dose calculation and how to inversly optimize directly 
...
% which are required for biological treatment planning using a variable RBE. Let's output the meta optimization parameter of the target structure:
ixTarget = 3;
cst{ixTarget,5}
...
% making it expensive for the optimizer to over and underdose the tumor (both are equaly important). 
cst{ixTarget,6}
...
%% Dose Calculation
% Lets generate dosimetric information by pre-computing dose influence matrices for unit pencil beam intensities that can be used 
% subsequently for inverse planning.
...
%% Now let's create another treatment plan this time use a coarser beam spacing.
% Instead of 30 degree spacing use a 50 degree spacing
