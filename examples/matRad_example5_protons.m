...
figure;
subplot(121);
imagesc(resultGUI.RBExDose_beam1(:, :, slice));
colorbar;
colormap(jet);
title('dose of beam 1');
subplot(122);
imagesc(resultGUI.RBExDose_beam2(:, :, slice));
colorbar;
colormap(jet);
title('dose of beam 2');
...
figure;
plot(profileOrginal, 'LineWidth', 2);
grid on;
hold on;
plot(profileShifted, 'LineWidth', 2);
