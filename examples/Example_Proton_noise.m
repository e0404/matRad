%% Example Proton Treatment Plan with extra noise in CT
...
%%
% Now let's add some gaussian noise on the ct
R = randn(ct.cubeDim);
noise = ct.cube{1} ./100 .*3 .*R;
ct.cube{1} = ct.cube{1} + noise;

%%
resultGUI_noise = matRad_calcDoseDirect(ct,stf,pln,cst,w);
...
figure
[hCMap,hDose,hCt,hContour,hIsoDose] = matRad_plotSliceWrapper(gca,ct,cst,1,resultGUI_noise.physicalDose,plane,slice,[],[],colorcube)
...
[gammaCube,gammaPassRateCell] = matRad_gammaIndex(...
    resultGUI_noise.physicalDose,resultGUI.physicalDose,...
