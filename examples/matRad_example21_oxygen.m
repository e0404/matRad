...
%%
pln.bioModel      = 'MKM';
resultGUI_B = matRad_calcDoseForward(ct, cst, stf, pln, resultGUI.w);
...
hold on;
title('RBExDose');
plot(resultGUI.RBExDose(:, 80, 80), 'DisplayName', 'LEM');
plot(resultGUI_B.RBExDose(:, 80, 80), 'DisplayName', 'MKM');
nexttile();
hold on;
title('Physical Dose');
plot(resultGUI.physicalDose(:, 80, 80), 'DisplayName', 'LEM');
% plot(resultGUI_B.physicalDose(:,80,80),'DisplayName','MKM')
legend();
