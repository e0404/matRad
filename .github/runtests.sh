...
${Runner} ${Switches} "back=cd('MOxUnit/MOxUnit'); moxunit_set_path; cd(back); cd('matRad'); matRad_rc; matRad_runTests('test'); exit(double(~ans))" > ../runtests.log #2> ../runtests.err put stdout to log, but let it print error messages
exitIfError $?
