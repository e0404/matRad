...
${Runner} ${Switches} "back=cd('MOxUnit/MOxUnit'); moxunit_set_path; cd(back); cd('matRad'); matRad_rc; moxunit_runtests('test','-junit_xml_file','testresults.xml'); exit(double(~ans))" > ../runtests.log #2> ../runtests.err put stdout to log, but let it print error messages
exitIfError $?
