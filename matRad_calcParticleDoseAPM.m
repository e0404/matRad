function dij = matRad_calcParticleDoseAPM(ct,stf,pln,cst,param)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if calcHeteroCorr
    calcHeteroCorrStruct.cube = {zeros(ct.cubeDFrim)};
...
for ShiftScen = 1:pln.multScen.totNumShiftScen
    ...
    for i = 1:numel(stf) % loop over all beams
        ...
        if calcHeteroCorr
            ...
            % interpolate hetero depth cube to dose grid resolution
                    ...
                    for ctScen = 1:pln.multScen.numOfCtScen
                        for RangeShiftScen = 1:pln.multScen.totNumRangeScen
                            ...
                            if pln.multScen.scenMask(ctScen,ShiftScen,RangeShiftScen)
                                ...
                                bixelDose = matRad_calcParticleDoseBixelWrapperAPM(...
                                    currRadDepths, ...
                                    isoLatDistsX(currIx),isoLatDistsY(currIx),...
                                    sigmaIni_sq, ...
                                    baseEntry,pln.bioParam.bioOpt,vTissueIndex_j(currIx,:),pln.propDoseCalc.calcLET,currHeteroCorrDepths);
