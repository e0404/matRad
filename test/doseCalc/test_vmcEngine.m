...
function test_vmcWorldCoordinatesAreRebasedToExportedCt
p = load('photons_testData.mat', 'ct');
ct = matRad_getWorldAxes(p.ct);
firstVoxelWorld = [ct.x(1), ct.y(1), ct.z(1)];

firstVoxelVmc = DoseEngines.matRad_PhotonVmcEngine.worldToVmcCoordinates(firstVoxelWorld, ct);
expected = [ct.resolution.x, ct.resolution.y, ct.resolution.z];
assertEqual(firstVoxelVmc, expected);

function test_vmcWindowsBatchUsesLegacyExecutableInterface
if ~ispc
    return
end

batchFile = [tempname, '.bat'];
cleanup = onCleanup(@() delete(batchFile));
matRad_createVmcBatchFile(1, batchFile, 0);
batchText = fileread(batchFile);

assertTrue(contains(batchText, '.\bin\vmc_Windows.exe MCpencilbeam_temp_1'));
assertFalse(contains(batchText, 'vmc_Windows.exe -i'));
clear cleanup;
