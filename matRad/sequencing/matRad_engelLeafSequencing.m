function resultGUI = matRad_engelLeafSequencing(resultGUI, stf, dij, numOfLevels, varargin)
% multileaf collimator leaf sequencing algorithm for intensity modulated
% beams with multiple static segments according to Engel et al. 2005
...
p.addRequired('dij', @(x) isstruct(x) || isempty(x));
...
for i = 1:numOfBeams
    ...
    while max(D_k(:) > 0)
        ...
        for j = 1:size(D_0, 1)
            ...
            data(j).right(1) = 0; % right internal limit, actual for an empty interval
...
% if weights exists from a former DAO remove it
