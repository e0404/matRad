    ...
    if ~isempty(doseInVoi)
        ...
        if strcmp(cst{runVoi, 3}, 'TARGET') > 0
            ...
            for runObjective = 1:numel(cst{runVoi, 6})
                ...
                % if strcmp(cst{runVoi,6}(runObjective).type,'square deviation') > 0 || ...
                %    strcmp(cst{runVoi,6}(runObjective).type,'square underdosing') > 0
            ...
            if referenceDose == inf
                ...
                VTarget95 = sum(doseInVoi >= 0.95 * referenceDose); % number of target voxels receiving dose >= 0.95 dPres
                VTreated95 = sum(doseCube(:) >= 0.95 * referenceDose);  % number of all voxels receiving dose >= 0.95 dPres ("treated volume")
