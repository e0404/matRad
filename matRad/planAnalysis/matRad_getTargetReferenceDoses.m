...
for refTarget = 1:numel(targetRows)
    ...
    refDose = matRad_getReferenceDoseFromObjectives(cst{cstIdx, 6});
    if isfinite(refDose)
        refDose = matRad_convertFromEvaluationMode(refDose, pln, 'total');
    else
...
function refDose = matRad_getReferenceDoseFromObjectives(objectives)
matRad_cfg = MatRad_Config.instance();
refDose = inf;
