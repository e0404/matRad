...
%|   "grandfathered: pre-existing complexity of this report generator, a long sequence of optional sections; restructuring is out of scope here");
...
if isfield(ct, 'dicomInfo')
    if isfield(ct.dicomInfo, 'PatientName')
        patientInformation.firstName = helper_parseFromDicom(ct.dicomInfo.PatientName, 'GivenName');
        patientInformation.lastName = helper_parseFromDicom(ct.dicomInfo.PatientName, 'FamilyName');
    end
    %%% in future release, patient sex and patient id might be moved in dicomInfo
    patientInformation.sex = helper_parseFromDicom(ct.dicomInfo, 'PatientSex');
    patientInformation.patientID = helper_parseFromDicom(ct.dicomInfo, 'PatientID');
else
...
%%% when id and sex moved to dicomInfo this gets obsolete
try
    ...
    for cubesToPlot = 1:3
        ...
        if cubesToPlot == 1
            ...
            matRad_plotSlice(ct, 'axesHandle', ax, 'cst', cst, 'cubeIdx', 1, 'dose', doseCube, ...
                             'plane', plane, 'slice', slice, 'contourColorMap', colors, ...
                             'colorBarLabel', colorMapLabel);
        ...
        elseif cubesToPlot == 2
            ...
            matRad_plotSlice(ct, 'axesHandle', ax, 'cst', cst, 'cubeIdx', 1, 'dose', doseCube, ...
                             'plane', plane, 'slice', slice, 'contourColorMap', colors, ...
                             'doseColorMap', gammaColormap, 'doseWindow', [0 2], ...
                             'colorBarLabel', colorMapLabel);
...
if exist('matRad_getGaussianOrbitSamples', 'file') == 2
    ...
    w(w == 0) = eps(class(w));
    matRad_createAnimationForLatexReport(confidenceValue, ct, cst, slice, doseStat.meanCubeW, sampDose, ...
                                         w, pln.subIx, framePath, legendColorbar, 'PrescribedDose', dPres);
...
for i = 1:size(cst, 1)
    if cst{i, 5}.Visible == true
        [y, argmin] = helper_cutAtArgmin(nominalScenario.dvh(i).volumePoints);
...
if response == 127 % means not found
    ...
    system(executeLatex); % Execute second time
...
function [y, argmin] = helper_cutAtArgmin(x)
[~, argmin] = min(x);
y = x(1:argmin);
end

function text = helper_parseFromDicom(dicomStruct, field, default)
