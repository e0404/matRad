classdef matRad_ParticleAnalyticalBortfeldEngine < DoseEngines.matRad_ParticlePencilBeamEngineAbstract
    ...
    properties (SetAccess = public, GetAccess = public)
        ...
        Z           = 10            % N of electrons per molecule (water) % mh:ignore_style
    ...
    methods (Access = public)
        ...
        function [doseVector, hatD] = calcAnalyticalBragg(this, primaryEnergy, depthZ, energySpread)
            ...
            % Number density of molecules per cm^3
            numberDensity   = this.massDensity * this.avogadroNum / this.MM;
            % Bohr's formula for d(sigmaE)^2/dz
            alphaPrime      = this.electronCharge^2 * numberDensity * this.Z / (4 * pi * this.epsilon0^2) / 10^8;
            ...
            % Squared Range straggling width
            sigmaMonoSquared = alphaPrime * this.p^2 * this.alpha^(2 / this.p) * range.^(3 - 2 / this.p) ./ (3 - 2 / this.p);
            sigmaMono = sqrt(sigmaMonoSquared); % Range straggling width

            % Compute the width of straggling, determined by widthMod
            % Total straggling contribution: range + energy
            sigmaTot = sqrt(sigmaMonoSquared + (energySpread^2) .* (this.alpha^2) .* (this.p^2) .* (primaryEnergy.^(2 * this.p - 2)));
            wid = sigmaTot;
            ...
            hatD   = (hatD1 + hatD2)  .* (depthZ <= range) + 0 .* (depthZ > range);  % Total depth-dose curve without straggling
    ...
    methods (Access = protected)
        ...
        function [currBixel] = getBixelIndicesOnRay(this, currBixel, currRay)
            ...
            elseif this.dosimetricLateralCutOff < 1 && this.dosimetricLateralCutOff > 0
                currIx = currRay.radDepths <= maxDepth + tmpOffset;
                sigmaSq = this.calcSigmaLatMCS(currRay.radDepths(currIx) - tmpOffset, currBixel.baseData.energy).^2 + currBixel.totalInitialSigmaSq;
        ...
        function X = interpolateKernelsInDepth(this, bixel) % mh:ignore_style
        ...
        function bixel = calcParticleBixel(this, bixel)
            ...
            sigmaSq = kernel.sigma.^2 + bixel.totalInitialSigmaSq;
        ...
        function calcLateralParticleCutOff(this, cutOffLevel, ~)
            ...
            for i = 1:numel(this.machine.data)
                ...
                mcsSigmaSq = this.calcSigmaLatMCS(this.machine.data(i).range, this.machine.data(i).energy)^2;
                this.machine.data(i).LatCutOff.CutOff = this.machine.data(i).LatCutOff.numSig * ...
                    sqrt(this.machine.data(i).LatCutOff.maxSigmaIni^2 + mcsSigmaSq);
            end
        end
