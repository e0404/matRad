classdef (Abstract) matRad_ParticlePencilBeamEngineAbstract < DoseEngines.matRad_PencilBeamEngineAbstract
    ...
    properties (SetAccess = public, GetAccess = public)
        ...
        calcBioDose = false             % Boolean which defines if biological dose calculation should be performed (alpha*dose and sqrt(beta)*dose)
    ...
    properties (SetAccess = protected, GetAccess = public)
        ...
        bioKernelQuantities              % Kernel quantities to request from the machine data for biological dose calculation

        restrictBeamRadDepthsByMaxEnergy = true
    end

    properties (Constant)
        rashiScatterRadiationModes = {'protons', 'helium', 'carbon', 'oxygen'}  % radiation modes for which range shifter scattering (matRad_calcSigmaRashi) is modeled
    end
    ...
    methods (Access = protected)
        ...
        function bixel = initBixel(this, currRay, k)
            ...
            % total initial sigma (squared) including additional
            % contributions from beam modifiers (e.g. range shifter
            % scattering). This is the spot size to use for lateral dose
            % calculation
            bixel.totalInitialSigmaSq = bixel.sigmaIniSq + bixel.addSigmaSq;
        ...
        function currBixel = getBeamModifiers(this, currBixel)
            addSigmaSq = 0;
            radDepthOffset = 0;
            % consider range shifter if applicable
            if currBixel.rangeShifter.eqThickness > 0
                ...
                if any(strcmp(this.machine.meta.radiationMode, this.rashiScatterRadiationModes))
                    % compute!
                    sigmaRashi = matRad_calcSigmaRashi(currBixel.baseData, ...
                                                       currBixel.rangeShifter, ...
                                                       currBixel.SSD, ...
                                                       this.machine.meta.radiationMode);

                    % add to initial sigma in quadrature
                    addSigmaSq = addSigmaSq + sigmaRashi.^2;
                else
                    matRad_cfg = MatRad_Config.instance();
                    matRad_cfg.dispWarning('Range shifter scattering not implemented for radiation mode ''%s'' and will be ignored!', this.machine.meta.radiationMode);
                end
        ...
        function dij = initDoseCalc(this, ct, cst, stf)
            ...
            if ~isa(this.bioModel, 'matRad_EmptyBiologicalModel') && ~isa(this.bioModel, 'matRad_ConstantRBE')
                ...
                % necessary. We could as well always do this calculation,
        ...
        function dij = allocateLETContainer(this, dij)
            ...
            % get MatLab Config instance for displaying warnings
            matRad_cfg = MatRad_Config.instance();
            if isfield(this.machine.data, 'LET')
        ...
        function calcLateralParticleCutOff(this, cutOffLevel, stfElement)
            % matRad function to calculate a depth-dependent lateral cutoff
            ...
            % find the largest initial beam width considering focus index, SSD and range shifter for each individual energy
            for i = 1:size(energySigmaLUT, 1)
                ...
                % consider range shifter if applicable
                if  any(strcmp(this.machine.meta.radiationMode, this.rashiScatterRadiationModes)) && rangeShifterLUT(i).eqThickness > 0
            ...
            for energyIx = vEnergiesIx
                ...
                for j = 1:numel(depthValues)
                    ...
                    if cutOffLevel == 1
                        ...
                        bixel.totalInitialSigmaSq = bixel.sigmaIniSq; % already includes range shifter contribution
            ...
            if this.visBoolLateralCutOff
                ...
                for kk = 1:numel(radDepths)
                    ...
                    % consider range shifter if applicable
                    if rangeShifter.eqThickness > 0 && any(strcmp(this.machine.meta.radiationMode, this.rashiScatterRadiationModes))

                        % compute!
                        sigmaRashi = matRad_calcSigmaRashi(this.machine.data(energyIx), rangeShifter, maxSSD, this.machine.meta.radiationMode);
                ...
                set(gcf, 'Color', [1 1 1]);
                subplot(311);
                h = imagesc(squeeze(mDose(midPos, :, :)));
                hold on;
                set(h, 'AlphaData', .8 * double(doseSlice > 0));
                contour(doseSlice, vLevelsDose, 'LevelListMode', 'manual', 'LineWidth', 2);
                hold on;
                ...
                legend({'isodose 1%,5%,10% 90%', 'calculated cutoff'});
                colorbar;
                set(gca, 'FontSize', 12);
                xlabel('z [mm]');
                ylabel('x [mm]');
                ...
                subplot(312);
                plot(this.machine.data(energyIx).depths, idd * conversionFactor, 'k', 'LineWidth', 2);
                grid on;
                hold on;
                plot(radDepths - this.machine.data(energyIx).offset, vDoseInt, 'r--', 'LineWidth', 2);
                hold on;
                plot(radDepths - this.machine.data(energyIx).offset, vDoseInt .* TmpCompFac, 'bx', 'LineWidth', 1);
                hold on;
                legend({'original IDD', ['cut off IDD at ' num2str(cutOffLevel) '%'], 'cut off IDD with compensation'}, 'Location', 'northwest');
                xlabel('z [mm]');
                ylabel('[MeV cm^2 /(g * primary)]');
                set(gca, 'FontSize', 12);
                ...
                if isfield(this.machine.data(energyIx), 'sigma1')
                    yyaxis left;
                    plot(this.machine.data(energyIx).LatCutOff.depths, this.machine.data(energyIx).LatCutOff.CutOff, 'LineWidth', 2);
                    hold on;
                    plot(this.machine.data(energyIx).depths, this.machine.data(energyIx).sigma1, ':', 'LineWidth', 2);
                    grid on;
                    hold on;
                    ylabel('mm');
                    yyaxis right;
                    plot(this.machine.data(energyIx).depths, this.machine.data(energyIx).sigma2, '-.', 'LineWidth', 2);
                    grid on;
                    hold on;
                    ylabel('mm');
                    ...
                    yyaxis right;
                    subplot(313);
                    plot(this.machine.data(energyIx).depths, this.machine.data(energyIx).sigma, 'LineWidth', 2);
                    grid on;
                    hold on;
                    legend({'Cutoff', 'sigma'});
                    ylabel('mm');
                ...
                cnt = 1;
                for i = vEnergiesIx
                    plot(this.machine.data(i).LatCutOff.depths, this.machine.data(i).LatCutOff.CutOff, 'LineWidth', 1.5);
                    hold on;
