...
%|                 "robustness and quantity dispatch in this core " +
%|                 "optimization routine; restructuring is out of scope " +
%|                 "for commits that merely touch it");
%| pragma Justify(metric, "cnest",
%|                 "grandfathered: pre-existing nesting of the " +
%|                 "per-structure, per-function, per-scenario loops; " +
%|                 "restructuring is out of scope for commits that " +
%|                 "merely touch it");
            ...
            if isa(objective, 'DoseObjectives.matRad_DoseObjective')
                ...
                switch robustness
                    ...
                    case 'PROB' % use the expectation value and the integral variance influence matrix
                        % First check the specific cache for probabilistic
                        ...
                        doseGradientExp{1}(cst{i, 4}{1}) = doseGradientExp{1}(cst{i, 4}{1}) + ...
                            objective.penalty * objective.computeDoseObjectiveGradient(d_i);
                    ...
                    case 'COWC' % composite worst case considers ovarall the worst objective function value
                        % First check the specific cache for COWC
                        ...
                        for s = 1:numel(useScen)
                            ...
                            delta_COWC{ixScen}(cst{i, 4}{ixContour}) = delta_COWC{ixScen}(cst{i, 4}{ixContour}) + ...
                                objective.penalty * objective.computeDoseObjectiveGradient(d_i);
                        end
                    ...
                    case 'OWC' % objective-wise worst case considers the worst individual objective function value
                        ...
                        for s = 1:numel(useScen)
                            ...
                            if fGrad(ixScen) ~= 0
                                doseGradient{ixScen}(cst{i, 4}{ixContour}) = doseGradient{ixScen}(cst{i, 4}{ixContour}) + ...
                                    fGrad(ixScen) * delta_OWC{ixScen}(cst{i, 4}{ixContour});
                            end
                        end
