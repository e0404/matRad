...
% matRad function to generate a vector representation of objects
...
%
% This is matRad_OptimizationProblemDAO's own (static, non-VMAT) version.
% matRad_OptimizationProblemVMAT < matRad_OptimizationProblemDAO overrides
% this same-named static method with a VMAT-aware version (different vector
% layout, includes inter-DAO-angle times); since static methods are called
% via an explicit class-qualified name (not polymorphic dispatch), VMAT
% plans always go through that override, never through this file.
...
for i = 1:size(apertureInfo.beam, 2)
    for j = 1:apertureInfo.beam(i).numOfShapes
        ...
        leafRange = offset + (1:apertureInfo.beam(i).numOfActiveLeafPairs);
        apertureInfoVec(leafRange) = apertureInfo.beam(i).shape(j).leftLeafPos;
        apertureInfoVec(leafRange + apertureInfo.totalNumOfLeafPairs) = apertureInfo.beam(i).shape(j).rightLeafPos;
