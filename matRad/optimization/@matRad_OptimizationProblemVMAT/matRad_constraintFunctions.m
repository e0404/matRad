...
leftLeafPos  = apertureInfoVec((1:nLeafTotal) + nShapes);
rightLeafPos = apertureInfoVec((1 + nLeafTotal + nShapes):(nShapes + nLeafTotal * 2));
...
if apertureInfo.continuousAperture
    ...
    c_lfspd = reshape([abs(diff(reshape(leftLeafPos, nLeafPairs, nShapes), 1, 2)) ...
                       abs(diff(reshape(rightLeafPos, nLeafPairs, nShapes), 1, 2))] ./ ...
                      repmat(timeBNOptAngles', nLeafPairs, 2), ...
                      2 * nLeafPairs * numel(timeBNOptAngles), 1);
end
