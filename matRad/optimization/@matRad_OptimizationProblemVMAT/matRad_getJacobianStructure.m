...
% frequently used sizes, hoisted to keep the index expressions readable
nShapes    = apertureInfo.totalNumOfShapes;
nLeafTotal = apertureInfo.totalNumOfLeafPairs;
...
if ~isempty(jacobStruct_dos_bixel)
    ...
    DAOBeams = find([apertureInfo.arc.beam.isDAOBeam]);
    ...
    constraintStride = kron((0:numOfConstraints - 1) * numel(apertureInfo.apertureVector), ...
                            ones(1, 2 * repFactor));
    % offset of the time variables within the aperture vector
    timeOffset = apertureInfo.totalNumOfShapes + apertureInfo.totalNumOfLeafPairs * 2;
    leafPosStride = repelem([0 apertureInfo.totalNumOfLeafPairs], 1, repFactor);
    ...
    for i = 1:numel(apertureInfo.beam)
        ...
        if apertureInfo.arc.beam(i).isDAOBeam
            ...
            % first weight
            jacobStructSparseVec(offset == j_sparse) = jacobStructSparseVec(offset == j_sparse) + weightContrib;
            ...
            for k = 1:apertureInfo.beam(i).numOfActiveLeafPairs
                ...
                jacobStructSparseVec(indInSparseVec) = jacobStructSparseVec(indInSparseVec) + ...
            ...
            lastDAOInd = find(DAOBeams == apertureInfo.arc.beam(i).lastDAOBeamIx, 1);
            ...
            nextDAOInd = find(DAOBeams == apertureInfo.arc.beam(i).nextDAOBeamIx, 1);
            ...
            for k = 1:apertureInfo.beam(i).numOfActiveLeafPairs
                ...
                % give fraction of gradient to previous optimized beam
                indInOptVec = apertureInfo.beam(apertureInfo.arc.beam(i).lastDAOBeamIx).shape(1).vectorOffset + k - 1;
                indInOptVec = repmat(indInOptVec, 1, repFactor) + leafPosStride;
                indInSparseVec = repmat(indInOptVec, 1, numOfConstraints) + constraintStride;
                ...
                % give the other fraction to next optimized beam
                indInOptVec = apertureInfo.beam(apertureInfo.arc.beam(i).nextDAOBeamIx).shape(1).vectorOffset + k - 1;
                indInOptVec = repmat(indInOptVec, 1, repFactor) + leafPosStride;
                indInSparseVec = repmat(indInOptVec, 1, numOfConstraints) + constraintStride;
            ...
            % give fraction of gradient to previous optimized beam
            lastDAOIndTime = find(DAOBeams == apertureInfo.arc.beam(i).lastDAOBeamIx, 1) + timeOffset;
            jacobStructSparseVec(lastDAOIndTime == j_sparse) = jacobStructSparseVec(lastDAOIndTime == j_sparse) + weightContrib;

            % give the other fraction to next optimized beam
            nextDAOIndTime = find(DAOBeams == apertureInfo.arc.beam(i).nextDAOBeamIx, 1) + timeOffset;
...
if apertureInfo.continuousAperture
    ...
    for i = 1:numel(apertureInfo.beam)
        ...
        if ~isempty(apertureInfo.arc.beam(i).leafConstMask)
            ...
            if apertureInfo.arc.beam(i).isDAOBeam
                ...
                % before
                i_sparse(indInSparseVec)    = indInConVec;
                j_sparse(indInSparseVec)    = apertureInfo.arc.beam(apertureInfo.arc.beam(i).lastDAOBeamIx).timeInd;
                indInSparseVec              = indInSparseVec + n;
                ...
                % after
                i_sparse(indInSparseVec)    = indInConVec;
                j_sparse(indInSparseVec)    = apertureInfo.arc.beam(apertureInfo.arc.beam(i).nextDAOBeamIx).timeInd;
                indInSparseVec              = indInSparseVec + n;
    ...
    jacobStruct_lfspd = sparse(i_sparse, j_sparse, s_sparse, 2 * n * nSpd, ...
                               numel(apertureInfo.apertureVector));
    ...
    timeInd = (1 + nShapes + nLeafTotal * 2):(nShapes * 2 + nLeafTotal * 2 - 1);
    nLeafBlock = nLeafPairs * numel(timeInd);
    timeVarOffset = nShapes + nLeafTotal * 2;

    currentLeftLeafInd = (nShapes + 1):(nShapes + nLeafBlock);
    currentRightLeafInd = (nShapes + nLeafTotal + 1):(nShapes + nLeafTotal + nLeafBlock);
    nextLeftLeafInd = (nLeafPairs + nShapes + 1):(nLeafPairs + nShapes + nLeafBlock);
    nextRightLeafInd = (nLeafPairs + nShapes + nLeafTotal + 1): ...
        (nLeafPairs + nShapes + nLeafTotal + nLeafBlock);
    leftTimeInd = kron(j, ones(1, nLeafPairs)) + timeVarOffset;
    rightTimeInd = kron(j, ones(1, nLeafPairs)) + timeVarOffset;
    % constraint index
    leafConstraintInd = 1:2 * nLeafBlock;

    % jacobian of the leafspeed constraint
    i = repmat((i' - 1) * nLeafPairs, 1, nLeafPairs) + ...
        repmat(1:nLeafPairs, 2 * numel(timeInd), 1);
    i = reshape([i' i' + nLeafBlock], 1, []);
    ...
    jacobStruct_lfspd = sparse(i, j, s, 2 * nLeafPairs * (nShapes - 1), ...
                               numel(apertureInfo.apertureVector), numel(s));
end

% jacobian of the doserate constraint
i = repmat(1:(apertureInfo.totalNumOfShapes), 1, 2);
j = [1:(apertureInfo.totalNumOfShapes) ...
     ((nShapes + nLeafTotal * 2) + 1):((nShapes + nLeafTotal * 2) + nShapes)];
