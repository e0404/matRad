...
matRad_cfg.dispInfo(['VMAT refinement: resampling %d beams (%d DAO control points) onto a %g deg grid, ' ...
                     'interpNew=%d reuseDij=%d continuousAperture=%d\n'], ...
                    numel(oldFineAngles), nnz([apertureInfo.arc.beam.isDAOBeam]), ...
                    angularSpacing, interpNew, reuseDij, continuousAperture);

stf = matRad_generateStf(ct, cst, pln);

matRad_cfg.dispInfo('VMAT refinement: %d -> %d beams, spacing %.4g -> %.4g deg\n', ...
                    numel(oldFineAngles), numel(stf), ...
                    abs(oldFineAngles(2) - oldFineAngles(1)), ...
                    abs(stf(2).gantryAngle - stf(1).gantryAngle));
